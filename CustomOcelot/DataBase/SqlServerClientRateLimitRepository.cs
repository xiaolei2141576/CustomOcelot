...
namespace CustomOcelot.DataBase
{
    public class SqlServerClientRateLimitRepository : IClientRateLimitRepository
    {
        ...
        public SqlServerClientRateLimitRepository(OcelotConfiguration options)
        {
            _options = options;
        }
        /// <summary>
        ...
        public async Task<(bool RateLimit, List<ClientRateLimitOptions> rateLimitOptions)> CheckClientRateLimitAsync(string clientid, string path)
        {
            using (var connection = new SqlConnection(_options.DbConnectionStrings))
            {
                ...
                var result = (await connection.QueryAsync<ClientRateLimitOptions>(sql, new { clientid, path }))?.AsList();
