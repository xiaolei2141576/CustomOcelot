...
namespace CustomOcelot.RateLimit
{
    ...
    public class ClientRateLimitProcessor : IClientRateLimitProcessor
    {
        ...
        private readonly IOcelotCache<ClientRateLimitCounter?> _clientRateLimitCounter;
        private readonly IClientRateLimitRepository _clientRateLimitRepository;
        private static readonly object _processLocker = new object();
        public ClientRateLimitProcessor(OcelotConfiguration options, IClientRateLimitRepository clientRateLimitRepository, IOcelotCache<ClientRateLimitCounter?> clientRateLimitCounter, IOcelotCache<ClientRoleModel> ocelotCache, IOcelotCache<RateLimitRuleModel> rateLimitRuleCache)
        {
            _options = options;
        ...
        {
            var result = false;
            var clientRule = new List<ClientRateLimitOptions>();
            //1、校验路由是否有限流策略
        ...
        /// <param name="path">请求地址</param>
        /// <returns></returns>
        private async Task<(bool RateLimit, List<ClientRateLimitOptions> rateLimitOptions)> CheckClientRateLimitAsync(string clientid, string path)
        {
        ...
        /// <returns></returns>
        private bool CheckRateLimitResult(List<ClientRateLimitOptions> rateLimitOptions)
        {
            bool result = true;
            ...
            {//校验策略
                ...
                {
                    ClientRateLimitCounter counter = new ClientRateLimitCounter(DateTime.UtcNow, 1);
                    //分别对每个策略校验
                    var enablePrefix = _options.RedisKeyPrefix + "RateLimitRule";
                    var key = OcelotHelper.ComputeCounterKey(enablePrefix, op.ClientId, op.Period, op.RateLimitPath);
                    var periodTimestamp = OcelotHelper.ConvertToSecond(op.Period);
                    lock (_processLocker)
                    {
                        ...
                        {//提取当前的计数情况
                         // 请求次数增长
                            ...
                            // 深拷贝
                            counter = new ClientRateLimitCounter(rateLimitCounter.Value.Timestamp, totalRequests);
                        }
                        else
                        {//写入限流策略
